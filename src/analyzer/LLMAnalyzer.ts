...
export class LLMAnalyzer {
  ...
  private readonly minCallInterval: number = 2200; // ~27 req/min, under Groq's 30 limit
  ...
  private async callLLM(prompt: string, retries: number = 3): Promise<string> {
    ...
    if (elapsed < this.minCallInterval) {
      await new Promise((resolve) =>
        setTimeout(resolve, this.minCallInterval - elapsed)
      );
    }
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        ...
        if (isRateLimit) {
          ...
          const waitTime = Math.max(15000, 5000 * Math.pow(2, attempt));
