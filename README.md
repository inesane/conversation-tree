...
Works in two modes:
- **Batch mode** — analyze a transcript after the fact
- **Live mode** — track topics in real-time as a conversation happens, with an interactive browser UI

## How It Works

1. Conversation text is chunked into 16-message sliding windows
2. Each window is sent to an LLM (Llama 3.3 70B via Groq, free) along with a compressed snapshot of the current topic tree
3. The LLM classifies whether each chunk continues the current topic, branches into a subtopic, goes on a tangent, or returns to a previous thread
4. Only high-confidence shifts (0.7+) are applied — filler, back-and-forth, and minor pivots are ignored
5. Each topic gets a running summary that captures key points, decisions, and open items
6. The result is an interactive tree you can explore in the browser or terminal

### Designed for Real Conversations

The tool handles natural dialogue of any length — from a 5-minute chat to a 15+ minute group discussion:
...
- **Running summaries** — every ~8 messages, the active topic gets a 2-4 sentence summary capturing key points and decisions
- **Rate limiting** — built-in throttling keeps usage under Groq's free tier limits

### Topic Classifications

- **Subtopic** — drilling deeper into the current topic. "Planning a trip" -> spending several messages on "the budget"
- **Tangent** — a digression triggered by association. "That reminds me..." — something that doesn't serve the main discussion
...
- **Paused** — was being discussed but the conversation moved away. These are the dropped threads you might want to return to
- **Completed** — topic was fully resolved (future feature)
- **Abandoned** — topic was intentionally dropped (future feature)

## Example

An 83-message conversation between three friends planning their weekend. The conversation drifts through tangents — a birthday party, a Netflix show, job talk, running into an old professor — before returning to finalize plans:
...
...
[Mia]: Saturday farmer's market and brunch, Sunday movie night. Great weekend
```
...
```bash
git clone git@github.com:inesane/conversation-tree.git
...
For live microphone mode (optional), also add a Deepgram key:
...
Start a real-time conversation tracker in the browser:

```bash
npx ts-node src/index.ts live
```

Opens `http://localhost:3000` with a split view — topic tree on the left, chat panel on the right. Type messages with different speaker names and watch the tree build as you go.

The tree updates automatically every 12 messages or 45 seconds. Click "Analyze Now" to trigger immediately. If you drift 3+ levels deep in tangents, a yellow alert banner appears.

#### Microphone mode (requires Deepgram)

```bash
npx ts-node src/index.ts live --mic
```

Captures audio from the browser, streams to Deepgram for real-time transcription with speaker diarization, then analyzes and updates the tree. Requires a free Deepgram API key from [console.deepgram.com](https://console.deepgram.com).
...
```bash
# Open the interactive tree in the browser
...
### Conversation format (batch mode)
...
├── tracker/
│   └── ConversationTracker.ts  # Pipeline orchestrator (batch + incremental)
├── live/
│   ├── LiveServer.ts           # Express + WebSocket server for live mode
│   ├── MessageBuffer.ts        # Buffers messages, triggers analysis cycles
│   └── Transcriber.ts          # Deepgram STT integration
├── cli/
...
public/
└── live.html                   # Real-time browser UI for live mode
...
Processing time includes rate limiting delays (2.2s between API calls). Actual LLM inference on Groq is fast (<1s per call). Live mode processes incrementally so there's no wait for the full conversation to finish.
